...
%% for 1c glitches
tic()
saveDir = '/home/deos/b.pinot/Documents/insight/glitches/saveDirTest/';
    ...
    if ~contains(filename, "3c") % to select only the glitches found by a 1c deglitching - depends on how the .mat files are named
        ...
        for i = 1:length(savedMat.glTimesUTC)
            fprintf(glitchList, ['%15s  %23s  %-.3e  %+.9e', blanks(3), ' %+.9e   %+.9e\n'], ...
                savedMat.channel(i,:), ...
                datetime(savedMat.glTimesUTC(i), 'ConvertFrom', 'datenum', 'Format', 'yyyy-MM-dd''T''HH:mm:ss.SSS'), ...
                savedMat.gldelay(i), ...
                savedMat.glAmpCounts(i), ...
                savedMat.y0(i), ...
                savedMat.slope(i));
        end
    end
